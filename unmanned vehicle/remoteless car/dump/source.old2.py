...
class GA:
    ...
    def generate_training_data(self, length=10, state_start=-10, state_stop=10, target_start=-10, target_stop=10):
        ...
        for x in range(length):
            val = [[random.randint(state_start, state_stop), 0, random.randint(state_start, state_stop)],
                   [random.randint(state_start, state_stop), 0, random.randint(state_start, state_stop)]]
    ...
    def evolve(self, state=0, target=10):
        ...
        while self.state != self.target:
            ...
            status = self.status()
            print("tried {} = {} => {}".format(t, self.state, status))
            if status == False:
                self.ri = random.randint(0, len(self.methods)-1)
...
##        print([(self.target[i] , self.state[i] , self.target[i] , self.prev_state[i]) for i in range(3)])
##        print([abs(self.target[i] - self.state[i]) < abs(self.target[i] - self.prev_state[i]) for i in range(3)])
    ...
    def process(self):
        ret = ""
        actions = self.methods[self.ri].split()
...
##    genetic_algorithm.learn()
    while True:
        genetic_algorithm.reply([int(x) for x in input("Enter start position: ").split()], [int(y) for y in input("Enter stop position: ").split()]) #make decision
        print()
